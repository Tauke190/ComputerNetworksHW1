...
int handleclientDataSocket(int sock);
...
void handleClientInput(int sock){
	...
	while (1)
	{
		...
		if(strcmp(client_command,"STOR") == 0){
			
			char filename[20];
			scanf("%s",filename);

			if(file_exists(filename)){
				int data_sock = handleclientDataSocket(sock);
				printf("Checking Authentication.....\n");
				char message[20] = "f";
				send(sock , message , sizeof(message),0); // Send file name
				...
				if(strcmp(server_response,"Valid User")==0){
					send(sock , filename , sizeof(filename),0); // Send file name
					send_to_data_socket(data_sock,filename); // Write the file on data socket stream
					recv(sock , &server_response , sizeof(server_response),0);
					printf("Server response: %s",server_response);
				}
				else {
		...
		if(strcmp(client_command,"RETR") == 0){
			...
			send(sock , filename , sizeof(filename),0); // Send the message identifier
			recv(sock , &server_response , sizeof(server_response),0); // File Found
			printf("Server response: %s",server_response);
		...
		if(strcmp(client_command,"data") == 0){
